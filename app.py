...
app = Flask(__name__)
app.secret_key = "sentinel_final_shield_2026"
DB_PATH = os.path.join(os.getcwd(), 'sentinel_v16.db')

# --- 静态配置 ---
DOMAINS = ["https://secure-link.top", "https://fb-check.net"]
COUNTRIES = ["中国", "香港", "台湾", "美国", "日本", "韩国", "新加坡", "马来西亚", "泰国", "阿联酋", "巴西", "英国", "德国"]
DEVICES = ["iPhone 13-15", "iPhone 16 Pro", "Android 14-15", "PC-Windows", "Mac-OS"]
...
def init_db():
    ...
    # 增加表单所需的所有字段：pixel, event, campaign, mock_req
...
<head>
    ...
    <style>
        ...
        .sidebar{width:220px; background:#fff; border-right:1px solid #e2e8f0; position:fixed; height:100vh;}
        .nav-link{display:block; padding:15px 25px; color:#64748b; font-weight:600; font-size:14px;}
        .nav-active{background:#f1f5f9; color:#2563eb; border-right:4px solid #2563eb;}
        .btn-blue{background:#2563eb; color:#fff; padding:10px 24px; border-radius:8px; font-weight:bold;}
        .toggle-box{padding:6px 12px; border:1px solid #e2e8f0; border-radius:6px; cursor:pointer; font-size:12px;}
        .selected{background:#2563eb; color:#fff;}
        input, select, textarea{width:100%; padding:10px; border:1px solid #e2e8f0; border-radius:8px; margin-top:5px; outline:none;}
        label{font-size:12px; font-weight:bold; color:#94a3b8; margin-top:10px; display:block; text-transform:uppercase;}
    </style>
</head>
<body class="flex">
    ...
    <main class="flex-1 ml-[220px] p-12">
        <div class="flex justify-between items-center mb-10">
            ...
            <button onclick="document.getElementById('m').style.display='flex'" class="btn-blue">+ 新增配置</button>
        </div>
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-50 border-b">
                    <tr>{% for h in headers %}<th class="p-4 text-slate-500">{{h}}</th>{% endfor %}<th class="p-4 text-right">操作</th></tr>
                </thead>
                <tbody class="divide-y">
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="p-4 text-right space-x-3">
                            <a href="/action/del/{{tab}}/{{row[0]}}" class="text-red-500 font-bold" onclick="return confirm('确定删除?')">删除</a>
                        </td>
                    </tr>
                    {% endfor %}
                </tbody>
        ...
        <div class="bg-white rounded-2xl w-full max-w-xl p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
            ...
            <form action="/action/add/{{tab}}" method="POST">
                {% if tab == 'tickets' %}
                    ...
                    <label>跳转目标 (输入WA号自动转链接)</label><input name="url" placeholder="如: 852xxxxxx" required>
                    <label>控制模式</label><select name="type"><option>单导模式</option><option>群导模式</option></select>
                    <label>风控规则</label><select name="p_id">{% for p in plist %}<option value="{{p[0]}}">{{p[1]}}</option>{% endfor %}</select>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div><label>广告事件</label><input name="event"></div>
                    </div>
                    <label>备注</label><textarea name="note" rows="2"></textarea>
                {% elif tab == 'policies' %}
                ...
                {% elif tab == 'links' %}
                    ...
                    <label>投放版位</label><input name="slot" placeholder="如: FB-Feed">
                    <label>备注</label><input name="note">
                {% endif %}
                <div class="flex justify-end mt-8 space-x-4 border-t pt-6">
                    <button type="button" onclick="document.getElementById('m').style.display='none'" class="text-slate-400">取消</button>
                    <button class="btn-blue px-10">确认保存</button>
                </div>
            </form>
        </div>
    </div>
...
def admin():
    ...
    conf = {
        ...
        "tickets": ("投放工单", ["ID", "名称", "链接", "像素"], "SELECT id, name, url, pixel FROM tickets"),
        "links": ("推广链路", ["ID", "访问路径", "版位", "备注"], "SELECT id, '/'||code, slot, note FROM mapping"),
        "logs": ("投放报表", ["ID", "链路", "IP", "状态", "详情", "版位", "备注", "时间"], "SELECT * FROM logs ORDER BY id DESC")
    }
